...
funcSrch={vid:'3nP6Tz45K_U',vnm:'',bs:false,bdb:true,player:null,idx:0,idxout:-1,bid:true,gre:'',gak:'AIzaSyCBrcqXgjh0hzFBS-nxUHMYUwvSEhFafng',tableID:'1AEwxOyi_yhIbmq6ymIougg5hL0uPxA6TBEH_OiGk',msk:[],tbsrch:[],
    pVid:function(i){var it;if(funcSrch.bid) {it=$("#entrer").jqxListBox('getItem', i);var tb=it['value'].split(',');funcSrch.vid=tb[1];funcSrch.vnm=tb[0];$("#entrer").jqxListBox({selectedIndex:funcSrch.idx});}else{it=$("#lrtrn").jqxListBox('getItem', i);funcSrch.idxout=it['index'];funcSrch.vid=it['value'];$("#lrtrn").jqxListBox({selectedIndex:funcSrch.idx});}funcSrch.player.loadVideoById(funcSrch.vid);},
    lent:function(src){$("#entrer").remove();$("#pl").append($("<div/>",{id:'entrer'}));$("#entrer").jqxListBox({allowDrop:true, source:src,width:w*13,height:h*0.95});$('#entrer').bind('select',function(event){funcSrch.idxout++;funcSrch.idx=event.args.index;var item=$('#entrer').jqxListBox('getItem',funcSrch.idx);var it=item['value'];var lb=it.split(',');funcSrch.vnm=lb[0].replace(' ','+');funcSrch.bid=true;funcSrch.player.loadVideoById(lb[1]);$("#lrtrn").jqxListBox('addItem',{label:lb[0],value:lb[1]});$('#listitem'+funcSrch.idxout+'lrtrn').jqxTooltip({content:lb[2]+"**//**"+lb[0]});});var items=$('#entrer').jqxListBox('getItems');var nb=0;$.each(items,function(){var lab=this['label'].split(',');$('#listitem'+nb+'entrer').attr("title",lab[2]+"**//**"+lab[0]);nb++;});funcSrch.bsip(src);},
    bsip:function(src){$("#tabsrch").jqxInput({width:w*9,height:'auto'});$('#tabsrch').on('change',function(event){if(event.args.type==='keyboard') funcSrch.ipsrch();});},
...
var rw=json["rows"];var val=$('#tabsrch').val().toLowerCase();for(var i in rw) if(rw[i][0].toLowerCase().contains(val)) funcSrch.msk.push([rw[i][2]+"/_/"+rw[i][0],rw[i][1]]);funcSrch.fnshipsrch();},
